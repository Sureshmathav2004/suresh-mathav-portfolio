...
export const assets = {
  ...
  frame: {
    ...
    image: '/assets/portrait/suresh.jpg' as string | null,
    fit: 'cover' as 'cover' | 'contain',
    position: '50% 22%',
    /* This is a plain photo, not luminance-keyed footage, so the key is off —
       otherwise the office background behind Suresh would be treated as a
       green-screen-style backdrop and partly erased. */
   ...
   */
  nameCutout: {
    /* Matted from the light-background avatar shot — a real alpha channel,
       so it now sits as a true silhouette in front of the moving type rather
       than a rectangle. One rough patch remains near the left ear where
       shadow on the backdrop was close to the hair's own tone; re-run
       `tools/extract-cutout.py`-style matting on a more evenly lit shot to
